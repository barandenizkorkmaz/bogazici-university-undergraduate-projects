...
# TO-DO: Handling exceptions based on packet 'TYPE'.
def send_packet(packet,target_ip,port):
    try:
...
def send_message():
    ...
    try:
        start_new_thread(send_packet,(packet,receiver_ip,PORT))
...
def service_manager(packet):
    try:
        ...
        if packet_type == "DISCOVER":
            ...
            start_new_thread(send_packet,(packet,sender_ip,PORT))
    ...
    if CHANNEL == 1:
        for i in range(255):
            ...
            if not(USER_IP == target_ip):
                ...
                start_new_thread(send_packet, (packet, target_ip, PORT))
    else:
...
listen_thread = threading.Thread(target=listen_port)
